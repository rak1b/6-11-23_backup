...
class ProductImageBase64(APIView):
    def get(self, request):
        ...
        baseStr = ""
        with open(img.barcode_modified.path, "rb") as img:
...
class ProductBarcodePrint(APIView):
    ...
    def get(self, request):
        try:
            slug = request.GET.get("slug","rickshaw-paint-theme-cushion-cover-soLvg")
            product = Products.objects.get(slug=slug)
            width, height = 400,350
            background_color = (255, 255, 255)
            image = Image.new("RGB", (width, height), background_color)

            # Define text, font sizes, and positions
            start_y = 15
            logo_data =  {"text": "KAARUJ", "font_size": 40, "position_y": start_y}

            price_data = {"text": f"{int(product.price)} BDT", "font_size":28, "position_y":logo_data["position_y"]+5+logo_data["font_size"]}

            vat_data = {"text": "(Vat Included)", "font_size": 18, "position_y": price_data["position_y"]+8+price_data["font_size"]}

            name_data = {"text": product.name[:30], "font_size": 24, "position_y": vat_data["position_y"]+10+vat_data["font_size"]}
            name_data_last = {"text": product.name[30:60], "font_size": 24, "position_y": name_data["position_y"]+3+name_data["font_size"]}

            if len(product.name) < 30:
                barcode_img_data = {"path": product.barcode.path,"font_size": 100, "position_y": name_data["position_y"]+20+name_data["font_size"]}
            else:
                barcode_img_data = {"path": product.barcode.path,"font_size": 110, "position_y": name_data_last["position_y"]+20+name_data_last["font_size"]}

            sku_data = {"text": product.sku, "font_size": 25, "position_y": barcode_img_data["position_y"]+5+barcode_img_data["font_size"]}

            if len(product.name) < 30:
                text_data = [logo_data,price_data,vat_data,name_data, sku_data]
            else:
                text_data = [logo_data,price_data,vat_data,name_data,name_data_last, sku_data]
            print(text_data)
            ...
            barcode_image = barcode_image.resize((width+50, barcode_img_data["font_size"]), Image.BILINEAR)
            ...
            # Create a Django HTTP response with the image
            response = HttpResponse(content=image_buffer.read(), content_type="image/png")
...
def insert_text(image, text, size, position_y=0, color=(0, 0, 0)):
    font_path = r"H:\Devsstream_NEW\kaaruj-backend-v2\media\bar_codes\products\base\font\FreeSansBold.ttf"
    # font_path = r"H:\Devsstream_NEW\kaaruj-backend-v2\media\bar_codes\products\base\font\arial_bold.ttf"
    draw = ImageDraw.Draw(image)
    font_size = size
    font = ImageFont.truetype(font_path, font_size)  # You can specify the font and size
...
class ProductView(ModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        if self.action == 'list':
            ...
            return ProductSerializer_Update
        elif self.action == 'outlet':
            return OutletProductSerializerForInvoice
        else:
    ...
    def get_serializer_context(self):
        ...
        if self.action == "outlet":
            context["outlet"] = Outlet.objects.get(id=self.kwargs["slug"])
    ...
    @paginate
    @action(detail=True, methods=['get'])
    def outlet(self, request,slug, *args, **kwargs):
        from sales.models import Outlet
        query = request.GET.get('query',"")
        product = request.GET.get('product',None)
        try:
            outlet_info = Outlet.objects.get(id=slug)
            products = outlet_info.get_products(query)
