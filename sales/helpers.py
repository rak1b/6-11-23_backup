...
from coreapp.email_utils import EmailThread

def sendPdfEmail(subject, message_text, from_mail, to_mail, pdf_content, params):
    try:
...
def SendEmailWithPDF(subject, message_text, from_mail, to_mail,params):
    try:
        template = get_template("invoice/index.html")
        html = template.render(params)
        pdf_content = BytesIO()
        pdf = pisa.pisaDocument(BytesIO(html.encode('UTF-8')), pdf_content)
        if pdf.err:
            print_log(f"Error in creating pdf for invoice {params['number']}")
            return
        sendPdfEmail(subject, message_text, from_mail, to_mail, pdf_content.getvalue(), params)
    except Exception as e:
        print_log('-----------Invoice Create Issue------------------')
        print_log(str(e))
...
def get_invoice_params(instance):
    ...
    try:
        params['payment_type'] = ["COD", "Card", "Bank", "Bkash","Sscommerz","Nagad","KOD","Rocket","Mixed"][instance.payment_type]
    except:
        params['payment_type'] = "N/A"
    ...
    for item in instance.get_invoice_products():
        params['invoice_products'].append({
            ...
            'variant_name': item.variant.name if item.variant else item.variant_name,
            'quantity': item.quantity,
            'price': item.product.get_offer_price if item.product else item.total,
            'quantity': item.quantity,
            'total': item.total,
            'thumb': item.variant.get_thumb_url if item.variant else settings.PLACEHOLDER_IMAGE,
        })
    return params
